...
//    $("#TableBody_F tr.NewRow").each(function (index) {
...
function ValidateSelectedItemRow() {
    ...
    let checkedRows =
        $("#TableBody_F .ItemCheckbox:checked");
...
$(document).on("click", ".ItemBatch_F", function (e) {
    ...
    let checkedCheckbox = $("#TableBody_F .CheckItem:checked");
        ...
        let ItemGridindex =
            $("#TableBody_F tr[data-rowid='" + rowID + "']").index();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var currentItemGridSelectedRow = GetCheckedRowId_F();
                ApplyBatchValues_F(currentItemGridSelectedRow);
                //#endregion
...
function ApplyBatchValues_F(rowId) {

    let batchValues = GetBatchValues_F(rowId);
...
function AddDeliveryNoteBatchRow(data = {}) {
    ...
    DeliveryNoteBatchList_F.push(batchObj);
...
function BindDeliveryNoteBatchTable() {
    ...
    $.each(DeliveryNoteBatchList_F, function (index, data) {
...
function ValidateBatchQty() {
    ...
    if (InvoiceQty !== BatchQty) {
        ...
        alert("Qty Mismatch !");
        var ItemGridRowSelected = GetCheckedRowId_F();
...
function GetBatchValues_F(rowId) {
...
function GetCheckedRowId_F() {

    let rowId = -1;

    $("#TableBody_F tr.NewRow:visible").each(function (index) {
...
//#region CONSUMPTION BATCH WIDTH (Batch WH tab)
...
//#region OTHER WH BATCH WIDTH
function ApplyOtherBatchFieldWidths(container = "#DeliveryNoteOtherBatchList") {
    const fields = [
        { cls: ".JIDNI_BCH_WH_Name", min: 10, max: 25, align: "left" },
        { cls: ".JIDNI_BCH_BatchDate", min: 10, max: 10, align: "center" },
        { cls: ".JIDNI_BCH_BatchNo", min: 20, max: 50, align: "left" },
        { cls: ".JIDNI_BCH_AvailableQty", min: 10, max: 20, align: "right" },
        { cls: ".JIDNI_BCH_BatchUnitPrice", min: 11, max: 20, align: "right" },
        { cls: ".JIDNI_BCH_BatchValue", min: 13, max: 25, align: "right" }
    ];
    ...
    fields.forEach(f => {
        const controls = $container.find(
            ...
            ", #DeliveryNoteOtherBatchTableBody > tr.DeliveryNoteOtherBatchRow " + f.cls
        );
        if (!controls.length) return;
