...
$(document).ready(function () {
    ...
    $(document).on("click", "#ItemTable_P input", function (e) {
        e.stopPropagation();
    ...
    $("#ItemTable_P").on("click", "tbody tr.NewRow", function (e) {
        ...
        $("#ItemTable_P .CheckItem").prop("checked", false);
    ...
    $("#ItemTable_P").on("click", ".CheckItem", function (e) {
    ...
    function autoAddRow_P (currentRow) {
        ...
        // allow only last row
        let isLastRow =
            currentRow.is("#ItemTable_P tbody tr.NewRow:last");
    ...
    $("#AddRowButton_P").on("click", function () {

        // 1. Validate last row before adding new row
        let isValid = true;


        $("#ItemTable_P tbody tr.NewRow:last").find("input, select").each(function () {
    ...
    $("#RemoveItemRowButton_P").on("click", function () {

        let checkedRows =
            $("#ItemTable_P tbody tr.NewRow:visible")
                .has(".CheckItem:checked");

        // minimum one row should exist
        let totalVisibleRows =
            $("#ItemTable_P tbody tr.NewRow:visible").length;
...
function DeleteItemRowTempTable(inputElement) {
    let ItemGridindex =
        $(inputElement)
            .closest("tr")
            .index("#ItemTable_P tbody tr.NewRow:visible") + 1;
    $.ajax({
...
function calculateTotal_P() {

    let totalQty = 0;
    let totalAmount = 0;

    // Loop through each row (only active rows)
    $("#ItemTable_P tbody tr.NewRow").each(function () {
...
function validateItemGrid() {
    ...
    $("#ItemTable_P tbody tr").each(function () {

        let row = $(this);

        // skip template row
        if (row.hasClass("TempRow_P")) return;
