...
namespace ERP.Controllers
{
            ...
            var addresses = new List<DeliveryNoteAddress_DTO>
    {
        ...
        public IActionResult Index()
        {
            ...
            return View("~/Views/JobworkInward/DeliveryNote/Index.cshtml");
        }
            ...
            String? PageFilter)
        {
            ...
            ViewBag.Collapse = true;
            return View(
          "~/Views/JobworkInward/DeliveryNote/DeliveryNoteSummary.cshtml",
    ...
    String? PageFilter)
       ...
       Convert.ToInt64(item["JIDNI_Number"]);
                ...
                dto.Items.Add(new DeliveryNoteItem_DTO
                {
                    ...
                    // Freight logic — without these, Edit page's checkbox/dropdown
                    // never bind since the DTO carries no saved value to render.
                    // FIXED: SP already selects the real column names
                    // (JIDNI_IsFreightApplicable etc.) — no need for the old
                    // Contains() check against stale names that never existed.
                    JIDNI_IsFreightApplicable =
                        item["JIDNI_IsFreightApplicable"] != DBNull.Value
                            ? Convert.ToString(item["JIDNI_IsFreightApplicable"])
                            : "No",

                    JIDNI_JIFRT_SVOH_Number =
                        item["JIDNI_JIFRT_SVOH_Number"] != DBNull.Value
                            ? Convert.ToInt64(item["JIDNI_JIFRT_SVOH_Number"])
                            : 0,

                    JIDNI_JIFRT_SVOI_Number =
                        item["JIDNI_JIFRT_SVOI_Number"] != DBNull.Value
                            ? Convert.ToInt64(item["JIDNI_JIFRT_SVOI_Number"])
                            : 0,

                    JIDNI_FromWH =
                        item.Table.Columns.Contains("JIDNI_FromWH") && item["JIDNI_FromWH"] != DBNull.Value
                            ? Convert.ToInt64(item["JIDNI_FromWH"])
                            : 0,

                    JIDNI_ToWH =
                        item.Table.Columns.Contains("JIDNI_ToWH") && item["JIDNI_ToWH"] != DBNull.Value
                            ? Convert.ToInt64(item["JIDNI_ToWH"])
                            : 0
                });
            }
