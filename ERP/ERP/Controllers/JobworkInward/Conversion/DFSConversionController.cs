    ...
    public class DFSConversionController : Controller
    {
        ...
        public IActionResult ConversionDefaultSetting()
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                SH_DTO.JIDNH_Number = Convert.ToInt64(row["JICNV_DFS_Number"]);
                SH_DTO.JIDNH_Shift_Number = Convert.ToInt64(row["JICNV_DFS_SFT_Number"]);
                SH_DTO.JIDNH_WC_Number = Convert.ToInt64(row["JICNV_DFS_WC_Number"]);
                SH_DTO.JIDNH_Operator_Number = Convert.ToString(row["JICNV_DFS_Operator"]);
                SH_DTO.JIDNH_PRS_Number = Convert.ToInt64(row["JICNV_DFS_PRS_Number"]);
                SH_DTO.JIDNH_MS_Number = Convert.ToInt64(row["JICNV_DFS_MS_Number"]);
            }

            ViewBag.Collapse = true;

            return View("~/Views/JobworkInward/Conversion/DFSConversion/ConversionDefaultSetting.cshtml", SH_DTO);
        }
        [HttpPost]
        ...
        public IActionResult GetConversion()
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                return Json(new
                {
                    ...
                    data = new
                    {
                        dfS_JICNVH_Number = row["JICNV_DFS_Number"],
                        dfS_JICNVH_SFT_Number = row["JICNV_DFS_SFT_Number"],
                        dfS_JICNVH_WC_Number = row["JICNV_DFS_WC_Number"],
                        dfS_JICNVH_Operator = row["JICNV_DFS_Operator"],
                        dfS_JICNVH_MS_Number = row["JICNV_DFS_MS_Number"],
                        dfS_JICNVH_PRS_Number = row["JICNV_DFS_PRS_Number"]
                    }
                });
            }
